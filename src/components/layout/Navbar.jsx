    ...
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled || isMobileMenuOpen
          ? 'bg-black/90 backdrop-blur-lg border-b border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.5)] py-2'
          : 'bg-transparent py-4'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        >
          <img
            src="/logo.png"
            alt="WebBees Logo"
            className="w-10 h-10 object-contain group-hover:scale-110 transition-transform drop-shadow-[0_0_8px_rgba(0,255,159,0.8)]"
          />
          <div className="text-2xl font-black tracking-tighter">
            <span className="text-white drop-shadow-md">Web</span>
            <span className="gradient-text">Bees</span>
        ...
        {/* Desktop Menu — all items including Catalog share the same pill pattern */}
        <div className="hidden md:flex items-center">
          <ul className="flex gap-4 lg:gap-6">
            {menuItems.map(({ title, href, icon, gradientFrom, gradientTo }, idx) => (
              <li
              ...
              >
                <span className="absolute inset-0 rounded-full bg-[linear-gradient(45deg,var(--gradient-from),var(--gradient-to))] opacity-0 transition-opacity duration-500 group-hover:opacity-100 mix-blend-screen pointer-events-none" />
                <span className="absolute inset-0 rounded-full bg-[linear-gradient(45deg,var(--gradient-from),var(--gradient-to))] blur-[15px] opacity-0 -z-10 transition-opacity duration-500 group-hover:opacity-60 pointer-events-none" />
                <span className="relative z-10 transition-transform duration-500 group-hover:scale-0 pointer-events-none text-white">
                  <span className="text-xl">{icon}</span>
                </span>
                <span className="absolute top-[14px] font-bold text-black uppercase tracking-wide text-xs transition-transform duration-500 scale-0 group-hover:scale-100 delay-150 pointer-events-none z-10">
                  {title}
            ...
            {/* Catalog pill — identical pattern, green gradient */}
            <li
              className="relative w-12 h-12 bg-black/50 border border-white/10 shadow-lg rounded-full flex items-center justify-center transition-all duration-500 hover:w-[120px] hover:border-transparent group cursor-pointer"
              onClick={() => navigate('/catalog')}
            >
              <span className="absolute inset-0 rounded-full bg-[linear-gradient(45deg,#00ff9f,#00e5ff)] opacity-0 transition-opacity duration-500 group-hover:opacity-100 mix-blend-screen pointer-events-none" />
              <span className="absolute inset-0 rounded-full bg-[linear-gradient(45deg,#00ff9f,#00e5ff)] blur-[15px] opacity-0 -z-10 transition-opacity duration-500 group-hover:opacity-60 pointer-events-none" />
              <span className="relative z-10 transition-transform duration-500 group-hover:scale-0 pointer-events-none text-white">
                <LayoutGrid className="w-5 h-5" />
              </span>
              <span className="absolute top-[14px] font-bold text-black uppercase tracking-wide text-xs transition-transform duration-500 scale-0 group-hover:scale-100 delay-150 pointer-events-none z-10 whitespace-nowrap">
                Catalog
              </span>
            </li>
          </ul>
          ...
          >
            <ul className="px-6 py-4 flex flex-col gap-4">
              {menuItems.map(({ title, href, icon, gradientFrom, gradientTo }, idx) => (
                <li key={idx}>
                  <button
                    className="w-full flex items-center gap-4 text-white hover:text-primary transition-colors p-2 rounded-lg hover:bg-white/5"
                    onClick={() => handleNavClick(href)}
                  >
                    <span
                      className="text-2xl p-2 rounded-lg"
              ...
              <li>
                <button
                  className="w-full flex items-center gap-4 text-white hover:text-[#00ff9f] transition-colors p-2 rounded-lg hover:bg-white/5"
                  onClick={() => { setIsMobileMenuOpen(false); navigate('/catalog'); }}
                >
                  <span
                    className="text-2xl p-2 rounded-lg"
                    style={{ background: 'linear-gradient(45deg,#00ff9f30,#00e5ff30)', color: '#00ff9f' }}
                  >
                    <LayoutGrid className="w-6 h-6" />
                  </span>
                  <span className="font-bold tracking-wide uppercase">Catalog</span>
                </button>
              </li>
            </ul>
          </motion.div>
